...
2. Two-dimensional autoregressive selectivity (year × age),
3. Three-dimensional autoregressive selectivity (year × age × cohort),
4. Independent semi-parametric deviations.
...
pol_model_var <- data.frame(
  cont_tv_fish_sel    = c("none_Fleet_1",
                          ...
                          "3dcond_Fleet_1",
                          "3dcond_Fleet_1"),
  fishsel_pe_pars_spec = c("none", rep("est_all", 3)),
  fish_sel_devs_spec   = c("none", rep("est_all", 3)),
  corr_opt_semipar     = c(rep(NA, 3), "corr_zero_y_b_c") 
)
```
...
```{r, eval = FALSE}
model_names <- c("constant", "2dar1", "3dgmrf", "iid")
...
fishsel_all_df <- fishsel_all_df %>% mutate(Model = factor(Model, levels = c("constant", "iid", "2dar1", "3dgmrf")))
ts_df <- ts_df %>% mutate(Model = factor(Model, levels = c("constant", "iid", "2dar1", "3dgmrf")))
```

Inspecting recruitment, the time series are generally consistent across models in both magnitude and trend. Spawning stock biomass estimates are also similar among models, though terminal year estimates differ slightly: the three-dimensional (`3d`) selectivity option yields the lowest biomass estimates, while the `constant` selectivity option produces the highest.
```{r, eval = FALSE}
