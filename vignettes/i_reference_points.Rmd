...
-   the recruitment sex ratio ($\psi_{y = y^{*},s}$),
...
per-recruit at an arbitrary value:
...
$$B_{x} = SS{BPR}^{\text{fished}} \cdot \frac{\sum_{RecYear}^{n_{y} - RecAge}{Rec}_{y,s}}{n_{y} - RecAge - RecYear}\psi_{y = y^{*},s}$$

where $SS{BPR}^{\text{fished}}\ $is multiplied by mean recruitment over
a user-defined period (i.e., $n_{y} - RecAge - RecYear$) and the
...
unfished individuals are set at an arbitrary value:
...
$$B_{msy} = SS{BPR}^{\text{fished}} \cdot EqRec \cdot \psi_{y = y^{*},s}$$
...
$$B_{r,x} = SS{BPR}_{r}^{\text{fished}} \cdot \frac{\sum_{RecYear}^{n_{y} - RecAge}{Rec}_{r,y,s}}{n_{y} - RecAge - RecYear}\psi_{r,y = y^{*},s}$$
...
-   the recruitment sex ratio ($\psi_{r,y = y^{*},s}$),
...
apportionment parameters ($\zeta_{r}$), which sum to 1:

$$N_{r,a = 1}^{\text{fished}} = \zeta_{r}$$

$$N_{r,a = 1}^{\text{unfished}} = \zeta_{r}$$
...
$$B_{r,msy} = SS{BPR}_{r}^{fished}{EqRec}_{r} \cdot \psi_{r,y = y^{*},s}$$
...
}{B_{r,msy} = SS{BPR}_{r}^{\text{fished}} \cdot {EqRec}_{r} \cdot \psi_{r,y = y^{*},s}}$$
...
each region is initialized with one recruit:
...
\left\lbrack - \left( NatMort_{r,y = y^{*},a - 1,s = 1} + \sum_{f}^{}\text{Frat}_{r,f} \cdot F_{r,msy} \cdot \text{Sel}_{r,y = y^{*},a - 1,s = 1,f}^{\text{Fsh}} \right) \right\rbrack, \text{if }2 < a < A \\
\end{matrix}$$
...
${OrigEqRec_o}$ until the contributions from all origins result in
self-consistent ${DestEqRec_r}$ values.
...
$$B_{r,msy} = \sum_{o}^{}{SS{BPR}_{o,r}^{\text{fished}} \cdot {OrigEqRec}_{o}} \cdot \psi_{r,y = y^{*},s}$$
...
Beyond traditional fishery reference points, SPoRC also computes dynamic
reference points. In particular, it estimates dynamic $B_{0}$, which
represents the unfished $SSB$ under time-varying demographic rates, and
is derived from the population projection equations with fishing
