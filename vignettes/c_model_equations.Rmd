...
ageing processes are applied sequentially. The population (or cohort) is
then projected forward to the penultimate age ($N_{r,a_{+} - 1,s}^{'}$),
which is used to derive the plus group. Specifically, the penultimate
age is projected forward once more:

$$\mathbf{X}_{s}\mathbf{=}\left( \left( \mathbf{N}_{a_{+} - 1,s}^{'} \right)^{T}\mathbf{M}_{y = 1,a_{+} - 1,s} \right) \odot \exp\left( - \mathbf{Z}_{a = a_{+} - 1,s}^{'} \right)$$

where $\mathbf{M}$ is a first-order Markov matrix representing movement,
$\odot$ is the Hadamard product (elementwise multiplication), and
