...
equations generally reduce by setting $r = 1$ and/or $s = 1$.

## Process Equations

### Population Initialization

In `SPoRC`, two primary methods exist to initialize the equilibrium
...
$$\begin{matrix}
N_{r,a,s}^{'} = \mu^{\text{Rec}}\frac{\exp\left( - \left( a_{+} - 1 \right) \cdot \left( Z_{r,a = a_{+},s}^{'} \right) \right)}{1 - exp\left( Z_{r,a = a_{+},s}^{'} \right)}\psi_{r,y = 1,s}\zeta_{r} \\
\end{matrix}$$

In contrast, the second method derives the initial population by
iterating the population to equilibrium. The second method for deriving
the initial population is well-suited when movement dynamics occur,
given that a closed-form solution cannot be easily solved for. Here, the
total instantaneous mortality in each region is defined similarly as in
the first method. An exponential decay model is used to initialize the
age structure at the first iteration:
...
$$\begin{matrix}
N_{r,y,a = 1,s} = \frac{4\mu^{\text{Rec}}\zeta_{r}h\sum_{r}^{}{SSB}_{r,y - RecLag}}{(1 - h)\sum_{r}^{}{SSB0_{r}} + 5(h - 1)\sum_{r}^{}{SSB}_{r,y - RecLag}}\exp\left( \epsilon_{r,y}^{\text{Rec}} - \frac{\sigma_{\text{Rec}}^{2}}{2}b_{y} \right)\psi_{r,y,s} \\
\end{matrix}$$
...
($\text{Catch}_{r,y,f}$) is then computed summation of the expected
catch-at-age across ages and sexes, multiplied by their respective
fishery weights-at-age ($W_{r,y,a,s,f}^{fish}$):
...
$$\begin{matrix}
\text{Recap}_{r,y,a,s}^{k} = \beta_{r,y}\frac{\sum_{f}^{}\text{Sel}_{r,y,a,s,f}^{\text{Fsh}}}{Z_{r,y,a,s}^{\text{Tag}}}T_{r,y,a,s}^{k}\left\lbrack 1 - exp\left( - Z_{r,y,a,s}^{\text{Tag}} \right) \right\rbrack \\
\end{matrix}$$
...
$$L\left( \epsilon_{r,y,i,s,j}^{\text{Sel}} \right) = \frac{1}{\sqrt{2\pi\sigma_{r,i,s,j,Sel}^{2}}\,}\exp\left( - \frac{\left( \epsilon_{r,y,i,j}^{\text{Sel}} \right)^{2}}{2\sigma_{r,i,s,j,Sel}^{2}} \right)$$
...
\frac{1}{\sqrt{2\pi \cdot 5^{2}}\,}\exp\left( - \frac{\left( \epsilon_{r,y = min(y^{j}),i,j}^{\text{Sel}} \right)^{2}}{2{\cdot 5}^{2}} \right),\ if\ y = min(y^{j}) \\
\frac{1}{\sqrt{2\pi\sigma_{r,i,s,j,Sel}^{2}}\,}\exp\left( - \frac{\left( \epsilon_{r,y,i,j}^{\text{Sel}} - \epsilon_{r,y - 1,i,j}^{\text{Sel}} \right)^{2}}{2\sigma_{r,i,s,j,Sel}^{2}} \right),if\ y > min(y^{j}) \\
\end{matrix} \right.\ $$

where process error deviations for the first year in which a given fleet
$j$ operates (i.e., $min(y^{j})$) are initialized with a large variance
(5). Following the first year, process error deviations follow a random
walk process with a mean conditional on the previous year's value
($\epsilon_{r,y - 1,i,j}^{\text{Sel}}$) and a variance of
$\sigma_{r,i,s,j,Sel}^{2}$.

In addition to being constrained by a normal distribution, both iid and
random walk cases have an additional smoothness penalty applied:

$$P_{SelYrSmoothness} = \sum_{r = 1}^{n_{r}}{\sum_{j = 1}^{n_{j}}{\sum_{s = 1}^{n_{s}}{\sum_{y = min(y^{j})}^{n_{y}}{\sum_{b = 1}^{n_{b}}\left( \log\left( {Sel}_{r,y,b,s,j} \right) - log({Sel}_{r,y - 1,b,s,j}) \right)^{2}}}}}$$
...
penalties are applied across bins and years to enforce curvature
control:

$$P_{SelBinCurve} = \sum_{r = 1}^{n_{r}}{\sum_{j = 1}^{n_{j}}{\sum_{s = 1}^{n_{s}}{\sum_{y = min(y^{j})}^{n_{y}}{\sum_{b = 2}^{n_{b} - 1}\left( \log\left( {Sel}_{r,y,b + 1,s,j} \right) - 2\log\left( {Sel}_{r,y,b,s,j} \right)\log\left( {Sel}_{r,y,b - 1,s,j} \right) \right)^{2}}}}}$$

$$P_{SelYrCurve} = \sum_{r = 1}^{n_{r}}{\sum_{j = 1}^{n_{j}}{\sum_{s = 1}^{n_{s}}{\sum_{y = \min\left( y^{j} \right) + 1}^{n_{y} - 1}{\sum_{b = 1}^{n_{b}}\left( \log\left( {Sel}_{r,y + 1,b,s,j} \right) - 2\log\left( {Sel}_{r,y,b,s,j} \right)\log\left( {Sel}_{r,y - 1,b,s,j} \right) \right)^{2}}}}}$$
