...
```{r, eval = F}
 # Setup survey selectivity and catchability
  ...
  map_ln_srvsel_devs <- array(as.numeric(input_list$map$ln_srvsel_devs), dim = dim(input_list$par$ln_srvsel_devs))
  map_ln_srvsel_devs[,,,,3] <- map_ln_srvsel_devs[,,,,2]
  input_list$map$ln_srvsel_devs <- factor(map_ln_srvsel_devs)
  input_list$data$map_ln_srvsel_devs <- map_ln_srvsel_devs
