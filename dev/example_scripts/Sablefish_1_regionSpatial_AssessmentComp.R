...
waa <- array(0, dim = c(dat$n_regions, length(dat$years), length(dat$ages), 2))
mataa <- array(0, dim = c(dat$n_regions, length(dat$years), length(dat$ages), 2))
agelen <- array(0, dim = c(dat$n_regions, length(dat$years), length(dat$length_bins),
                           length(dat$ages), 2))

for(y in 1:length(dat$years)) {

  # weight at age
  waa[1,y,,1] <- dat$female_mean_weight_by_age[,y]
  waa[1,y,,2] <- dat$male_mean_weight_by_age[,y]

  # maturity at age
  mataa[1,y,,1] <- dat$maturity[,y]
  mataa[1,y,,2] <- dat$maturity[,y]

  # age length transition
  agelen[1,y,,,1] <- t(dat$female_age_length_transition[,,y])
  agelen[1,y,,,2] <- t(dat$male_age_length_transition[,,y])

} # end y loop


# Catch
obscatch <- array(0, dim = c(dat$n_regions, length(dat$years), 2))
usecatch <- array(0, dim = c(dat$n_regions, length(dat$years), 2))
catchtype <- array(1, dim = c(length(dat$years), 2))

obscatch[1,,1] <- as.vector(dat$fixed_fishery_catch) # fixed gear catches
obscatch[1,,2] <- as.vector(dat$trwl_fishery_catch) # trawl gear catches
obscatch[1,1:3,2] <- 0 # set first three years catch as 0
usecatch[obscatch != 0] <- 1 # use catch indicator

# fishery index
obsfishidx <- array(0, dim = c(dat$n_regions, length(dat$years), 2))
obsfishidx_se <- array(0, dim = c(dat$n_regions, length(dat$years), 2))
usefishidx <- array(0, dim = c(dat$n_regions, length(dat$years), 2))

# fishery ages
obsfishage <- array(0, dim = c(dat$n_regions, length(dat$years), length(dat$ages), 2, 2))
usefishage <- array(0, dim = c(dat$n_regions, length(dat$years), 2))
issfishage <- array(0, dim = c(dat$n_regions, length(dat$years), 2, 2))

for(y in 1:length(dat$years)) {
  obsfishage[1,y,,1,1] <- dat$obs_fixed_catchatage[-c(1:30),,y] # females
  obsfishage[1,y,,2,1] <- dat$obs_fixed_catchatage[c(1:30),,y] # males
}

usefishage[1,,1] <- dat$fixed_catchatage_indicator # use catch at age indicator
issfishage[1,which(dat$fixed_catchatage_indicator == 1),1,1] <- 250 # iss weighting for catch at age

# fishery lengths
obsfishlen <- array(0, dim = c(dat$n_regions, length(dat$years), length(dat$length_bins), 2, 2))
usefishlen <- array(0, dim = c(dat$n_regions, length(dat$years), 2))
issfishlen <- array(0, dim = c(dat$n_regions, length(dat$years), 2, 2))

for(y in 1:length(dat$years)) {

  # fixed gear
  obsfishlen[1,y,,1,1] <- dat$obs_fixed_catchatlgth[-c(1:30),,y] # females
  obsfishlen[1,y,,2,1] <- dat$obs_fixed_catchatlgth[c(1:30),,y] # males

  # trawl gear
  obsfishlen[1,y,,1,2] <- dat$obs_trwl_catchatlgth[-c(1:30),,y] # females
  obsfishlen[1,y,,2,2] <- dat$obs_trwl_catchatlgth[c(1:30),,y] # males
}

# fixed gear
usefishlen[1,,1] <- dat$fixed_catchatlgth_indicator # use catch at length indicator
issfishlen[1,which(dat$fixed_catchatlgth_indicator == 1),1,1] <- 200 # iss weighting for catch at length

# trawl gear
usefishlen[1,,2] <- dat$trwl_catchatlgth_indicator # use catch at length indicator
issfishlen[1,which(dat$trwl_catchatlgth_indicator == 1),1,2] <- 200 # iss weighting for catch at length

# survey index
obssrvidx <- array(0, dim = c(dat$n_regions, length(dat$years), 2))
obssrvidx_se <- array(0, dim = c(dat$n_regions, length(dat$years), 2))
usesrvidx <- array(0, dim = c(dat$n_regions, length(dat$years), 2))
...
# survey ages
obssrvage <- array(0, dim = c(dat$n_regions, length(dat$years), length(dat$ages), 2, 2))
usesrvage <- array(0, dim = c(dat$n_regions, length(dat$years), 2))
isssrvage <- array(0, dim = c(dat$n_regions, length(dat$years), 2, 2))

for(y in 1:length(dat$years)) {

  # domestic
  obssrvage[1,y,,1,1] <- dat$obs_srv_catchatage[-c(1:30),,y,1] # females
  obssrvage[1,y,,2,1] <- dat$obs_srv_catchatage[c(1:30),,y,1] # males

  # japanese
  obssrvage[1,y,,1,2] <- dat$obs_srv_catchatage[-c(1:30),,y,2] # females
  obssrvage[1,y,,2,2] <- dat$obs_srv_catchatage[c(1:30),,y,2] # males
}

usesrvage[1,,] <- dat$srv_catchatage_indicator # use catch at age indicator
isssrvage[] <- 300 # iss weighting for catch at age

# survey lengths
obssrvlen <- array(0, dim = c(dat$n_regions, length(dat$years), length(dat$length_bins), 2, 2))
usesrvlen <- array(0, dim = c(dat$n_regions, length(dat$years), 2))
isssrvlen <- array(0, dim = c(dat$n_regions, length(dat$years), 2, 2))
