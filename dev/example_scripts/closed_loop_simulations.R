# Purpose: To demonstrate how to run a closed loop simulation without any stock assessment error, conditioned on a SPoRC model
...
# Setup Reference Points and Projection Options ---------------------------

# reference points options
reference_points_opt <- list(
  n_avg_yrs = 1, # number of years to average over demographic rates
  SPR_x = 0.4, # SPR rates
  calc_rec_st_yr = 3, # year to use recruitment for bx% calculations
  rec_age = 4, # recruitment age
  ...
  for (y in 1:sim_env$n_yrs) {
    ...
    if(y >= sim_env$feedback_start_yr) {
     ...
     if(y %in% assessment_years) {
       ...
       tmp_terminal_NAA0 <- array(obj$rep$NAA0[,y,,], dim = c(asmt_data$n_regions, length(asmt_data$ages), asmt_data$n_sexes)) # terminal numbers at age
...
png(here("vignettes", "figures", "h_fmort_closed_loop.png"), width = 800, height = 400)
