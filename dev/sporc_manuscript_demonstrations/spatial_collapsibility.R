...
n_fish_fleets <- 2
do_recruits_move <- 0
terminal_NAA <- array(sgl_rep$NAA[,length(sgl_dat$years),,], dim = c(n_regions, n_ages, n_sexes))
terminal_NAA0 <- array(sgl_rep$NAA0[,length(sgl_dat$years),,], dim = c(n_regions, n_ages, n_sexes))
WAA <- array(rep(sgl_dat$WAA[,length(sgl_dat$years),,], each = n_proj_yrs), dim = c(n_regions, n_proj_yrs, n_ages, n_sexes)) # weight at age
WAA_fish <- array(rep(sgl_dat$WAA[,length(sgl_dat$years),,], each = n_proj_yrs), dim = c(n_regions, n_proj_yrs, n_ages, n_sexes, n_fish_fleets)) # weight at age
MatAA <- array(rep(sgl_dat$MatAA[,length(sgl_dat$years),,], each = n_proj_yrs), dim = c(n_regions, n_proj_yrs, n_ages, n_sexes)) # maturity at age
...
Movement <- NULL
terminal_F <- array(sgl_rep$Fmort[,length(sgl_dat$years),], dim = c(n_regions, n_fish_fleets))
...
n_fish_fleets <- 2
do_recruits_move <- 0
terminal_NAA <- array(three_rep$NAA[,length(three_dat$years),,], dim = c(n_regions, n_ages, n_sexes))
terminal_NAA0 <- array(three_rep$NAA0[,length(three_dat$years),,], dim = c(n_regions, n_ages, n_sexes))
WAA <- array(rep(three_dat$WAA[,length(three_dat$years),,], each = n_proj_yrs), dim = c(n_regions, n_proj_yrs, n_ages, n_sexes)) # weight at age
WAA_fish <- array(rep(three_dat$WAA[,length(sgl_dat$years),,], each = n_proj_yrs), dim = c(n_regions, n_proj_yrs, n_ages, n_sexes, n_fish_fleets)) # weight at age
MatAA <- array(rep(three_dat$MatAA[,length(three_dat$years),,], each = n_proj_yrs), dim = c(n_regions, n_proj_yrs, n_ages, n_sexes)) # maturity at age
...
Movement <- aperm(abind::abind(replicate(n_proj_yrs, three_rep$Movement[,,length(three_dat$years),,], simplify = FALSE), along = 5), perm = c(1,2,5,3,4))
terminal_F <- array(three_rep$Fmort[,length(three_dat$years),], dim = c(n_regions, n_fish_fleets))
...
WAA_fish <- array(rep(five_dat$WAA[,length(sgl_dat$years),,], each = n_proj_yrs), dim = c(n_regions, n_proj_yrs, n_ages, n_sexes, n_fish_fleets)) # weight at age
...
ggsave(
  here("dev", "sporc_manuscript_demonstrations", "figs", "spatial_proj.png"),
  print(
...
ggsave(
  here("dev", "sporc_manuscript_demonstrations", "figs", "spatial_proj_presentation.png"),
  all_ssb_plot + theme(legend.position = c(0.15, 0.125)),
  width = 10, height = 8
)

ggsave(
  here("dev", "sporc_manuscript_demonstrations", "figs", "three_area_spatial_proj_presentation.png"),
  three_ssb_plot,
  width = 5, height = 8
)
...
for(t in 2:n_t) {
  ...
  if(t <= dim(three_rep$Movement)[4]) NAA_three[,t] <- NAA_three[,t-1] %*% three_rep$Movement[,,1,t,1]
  else NAA_three[,t] <- NAA_three[,t-1] %*% three_rep$Movement[,,1,30,1]

  # five region stationary movement
  if(t <= dim(five_rep$Movement)[4]) NAA_five[,t] <- NAA_five[,t-1] %*% five_rep$Movement[,,1,t,1]
  else NAA_five[,t] <- NAA_five[,t-1] %*% five_rep$Movement[,,1,30,1]
...
ggsave(
  here("dev", "sporc_manuscript_demonstrations", "figs", "move_proj.png"),
  print(
...
ggsave(
  here("dev", "sporc_manuscript_demonstrations", "figs", "rec_presentation.png"),
...
ggsave(
  here("dev", "sporc_manuscript_demonstrations", "figs", "spatial_hcr.png"),
  multi_hcr_plots,
  height = 13, width = 8
)

ggsave(
  here("dev", "sporc_manuscript_demonstrations", "figs", "spatial_hcr_presentation.png"),
