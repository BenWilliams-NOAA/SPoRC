...
# Read in data
tem_dat <- dget(here("dev", '2024 Base (23.5)_final model', 'tem.rdat'))
ageing_dat <- dget(here("dev",'2023 Base (23.5)_final model', 'test.rdat')) # for getting ageing error
tem_admb_dat <- readLines(here("dev","2024 Base (23.5)_final model", "tem_2024_na_wh.dat"))
tem_rep <- readLines(here("dev",'2024 Base (23.5)_final model', 'sable.rep'))
tem_par <- read_pars(here("dev",'2024 Base (23.5)_final model', 'tem'))
...
data$bias_year <- c(length(1960:1979), length(1960:1989), (length(1960:2023) - 5), length(1960:2024) - 2)
...
data$sablefish_ADMB = 1 # share fishery index selectivity for the first time block and fleet across sexes (specific to sablefish)
...
data$Wt_FishAgeComps <- array(NA, dim = c(data$n_regions, length(data$years), data$n_sexes, data$n_fish_fleets)) # weights for fishery age comps
data$Wt_FishAgeComps[1,,1,1] <- 0.826107286513784 # Weight for fixed gear age comps
...
data$Wt_FishLenComps <- array(0, dim = c(data$n_regions, length(data$years), data$n_sexes, data$n_fish_fleets)) # weights for fishery age comps
data$Wt_FishLenComps[1,,1,1] <- 4.1837057381917 # Weight for fixed gear len comps females
data$Wt_FishLenComps[1,,2,1] <- 4.26969350917589 # Weight for fixed gear len comps males
data$Wt_FishLenComps[1,,1,2] <- 0.316485920691651 # Weight for trawl gear len comps females
data$Wt_FishLenComps[1,,2,2] <- 0.229396580680981 # Weight for trawl gear len comps males
...
data$Wt_SrvAgeComps <- array(NA, dim = c(data$n_regions, length(data$years), data$n_sexes, data$n_srv_fleets)) # weights for survey age comps
data$Wt_SrvAgeComps[1,,1,1] <- 3.79224544725927 # Weight for domestic survey ll gear age comps
data$Wt_SrvAgeComps[1,,1,3] <- 1.31681114024037 # Weight for coop jp survey ll gear age comps
...
# data weights
data$Wt_SrvLenComps <- array(0, dim = c(data$n_regions, length(data$years), data$n_sexes, data$n_srv_fleets)) # weights for survey age comps
data$Wt_SrvLenComps[1,,1,1] <- 1.43792019016567 # Weight for domestic ll survey len comps females
data$Wt_SrvLenComps[1,,2,1] <- 1.07053763450712 # Weight for domestic ll survey len comps males
data$Wt_SrvLenComps[1,,1,2] <- 0.670883273592302 # Weight for domestic trawl survey len comps females
data$Wt_SrvLenComps[1,,2,2] <- 0.465207132450763 # Weight for domestic trawl survey len comps males
data$Wt_SrvLenComps[1,,1,3] <- 1.27772810174693 # Weight for coop jp ll survey len comps females
data$Wt_SrvLenComps[1,,2,3] <- 0.857519546948587 # Weight for coop jp ll survey len comps males
...
### Survey Selectivity ------------------------------------------------------
# Time Block Specification
data$cont_tv_srv_sel <- array(0, dim = c(data$n_regions, data$n_srv_fleets)) # no timevarying selex continously
...
# catchability stuff
data$Use_srv_q_prior = 0
data$Use_fish_q_prior = 0
...
parameters$fishsel_pe_pars <- array(0, dim = c(data$n_regions, 4, data$n_sexes, data$n_fish_fleets))
...
# Set up continuous srvery selectivity stuff
# process error parameters (sigmas and corrs) (using 4 as the max number of pars that can deviate, and then just map off if not using)
parameters$srvsel_pe_pars <- array(0, dim = c(data$n_regions, 4, data$n_sexes, data$n_srv_fleets))
# process error deviations (using ages as the max number of pars that can deviate, and then just map off if not using)
parameters$ln_srvsel_devs <- array(0, dim = c(data$n_regions, length(data$years), length(data$ages), data$n_sexes, data$n_srv_fleets))
...
parameters$ln_fish_fixed_sel_pars[1,,1,1,1] <- c(tem_par$coefficients[names(tem_par$coefficients) == "log_a50_fish1_f"], tem_par$coefficients[names(tem_par$coefficients) == "log_delta_fish1_f"]) # Female longline fishery first time block parameters (a50 and then delta)
parameters$ln_fish_fixed_sel_pars[1,,2,1,1] <- c(tem_par$coefficients[names(tem_par$coefficients) == "log_a50_fish4_f"], tem_par$coefficients[names(tem_par$coefficients) == "log_delta_fish4_f"]) # Female longline fishery second time block parameters (a50 and then delta)
parameters$ln_fish_fixed_sel_pars[1,,3,1,1] <- c(tem_par$coefficients[names(tem_par$coefficients) == "log_a50_fish5_f"], tem_par$coefficients[names(tem_par$coefficients) == "log_delta_fish5_f"]) # Female longline fishery third time block parameters (a50 and then delta)

parameters$ln_fish_fixed_sel_pars[1,,1,2,1] <- c(tem_par$coefficients[names(tem_par$coefficients) == "log_a50_fish1_m"], tem_par$coefficients[names(tem_par$coefficients) == "log_delta_fish1_f"]) # Male longline fishery first time block parameters (a50 and then delta)
parameters$ln_fish_fixed_sel_pars[1,,2,2,1] <- c(tem_par$coefficients[names(tem_par$coefficients) == "log_a50_fish4_m"], tem_par$coefficients[names(tem_par$coefficients) == "log_delta_fish4_m"]) # Male longline fishery second time block parameters (a50 and then delta)
parameters$ln_fish_fixed_sel_pars[1,,3,2,1] <- c(tem_par$coefficients[names(tem_par$coefficients) == "log_a50_fish5_m"], tem_par$coefficients[names(tem_par$coefficients) == "log_delta_fish5_m"]) # Male longline fishery third time block parameters (a50 and then delta)

# Trawl Fishery kept the same across blocks
parameters$ln_fish_fixed_sel_pars[1,,1,1,2] <- c(tem_par$coefficients[names(tem_par$coefficients) == "log_a50_fish3_f"], tem_par$coefficients[names(tem_par$coefficients) == "log_delta_fish3_f"]) # Female trawl fishery first time block parameters (amax and then power)
parameters$ln_fish_fixed_sel_pars[1,,2,1,2] <- c(tem_par$coefficients[names(tem_par$coefficients) == "log_a50_fish3_f"], tem_par$coefficients[names(tem_par$coefficients) == "log_delta_fish3_f"]) # Female trawl fishery first time block parameters (amax and then power)
parameters$ln_fish_fixed_sel_pars[1,,3,1,2] <- c(tem_par$coefficients[names(tem_par$coefficients) == "log_a50_fish3_f"], tem_par$coefficients[names(tem_par$coefficients) == "log_delta_fish3_f"]) # Female trawl fishery first time block parameters (amax and then power)

# Trawl Fishery kept the same across blocks
parameters$ln_fish_fixed_sel_pars[1,,1,2,2] <- c(tem_par$coefficients[names(tem_par$coefficients) == "log_a50_fish3_m"], tem_par$coefficients[names(tem_par$coefficients) == "log_delta_fish3_f"]) # Male trawl fishery first time block parameters (amax and then power)
parameters$ln_fish_fixed_sel_pars[1,,2,2,2] <- c(tem_par$coefficients[names(tem_par$coefficients) == "log_a50_fish3_m"], tem_par$coefficients[names(tem_par$coefficients) == "log_delta_fish3_f"]) # Male trawl fishery first time block parameters (amax and then power)
parameters$ln_fish_fixed_sel_pars[1,,3,2,2] <- c(tem_par$coefficients[names(tem_par$coefficients) == "log_a50_fish3_m"], tem_par$coefficients[names(tem_par$coefficients) == "log_delta_fish3_f"]) # Male trawl fishery first time block parameters (amax and then power)
...
parameters$ln_fish_q[1,3,1] <- tem_par$coefficients[names(tem_par$coefficients) == "log_q_LL_fish_recent"] # third fixed gear fishery time block
...
parameters$ln_srv_fixed_sel_pars[1,,,1,3] <- c(0.980660760456, tem_par$coefficients[names(tem_par$coefficients) == "log_delta_srv1_f"]) # Female trawl survey first time block parameters (a50, delta)
parameters$ln_srv_fixed_sel_pars[1,,,2,3] <- c(1.22224502478, tem_par$coefficients[names(tem_par$coefficients) == "log_delta_srv1_m"]) # Male trawl survey first time block parameters (a50, delta)
...
parameters$ln_RecDevs <- array(c(tem_par$coefficients[str_detect(names(tem_par$coefficients), "rec_dev")][-c(1:28)]),
                               dim = c(data$n_regions, length(data$years) - 1))
parameters$ln_sigmaR <- c(log(0.4), tem_par$coefficients[names(tem_par$coefficients) == "log_sigr"] ) # early sigma R
parameters$h <- c(10)
...
parameters$Tag_Reporting_Pars <- array(log(0.2 / (1 - 0.2)), dim = c(data$n_regions, length(unique(as.vector(data$Tag_Reporting_blocks)))))
parameters$ln_tag_theta <- log(0.1)

# Comp Likelihood Stuff ---------------------------------------------------
parameters$ln_FishAge_theta <- array(0, dim = c(data$n_regions, data$n_sexes, data$n_fish_fleets))
parameters$ln_FishLen_theta <- array(0, dim = c(data$n_regions, data$n_sexes, data$n_fish_fleets))
parameters$ln_SrvAge_theta <- array(0, dim = c(data$n_regions, data$n_sexes, data$n_srv_fleets))
parameters$ln_SrvLen_theta <- array(0, dim = c(data$n_regions, data$n_sexes, data$n_srv_fleets))
parameters$ln_FishAge_theta_agg <-array(0, dim = data$n_fish_fleets)
parameters$ln_FishLen_theta_agg <- array(0, dim = data$n_fish_fleets)
parameters$ln_SrvAge_theta_agg <-array(0, dim = data$n_srv_fleets)
parameters$ln_SrvLen_theta_agg <- array(0, dim = data$n_srv_fleets)
parameters$SrvLen_corr_pars_agg <- array(0.01, dim = c(data$n_srv_fleets))
parameters$SrvLen_corr_pars <- array(0.01, dim = c(data$n_regions, data$n_sexes, data$n_srv_fleets, 3))
parameters$SrvAge_corr_pars_agg <- array(0.01, dim = c(data$n_srv_fleets))
parameters$SrvAge_corr_pars <- array(0.01, dim = c(data$n_regions, data$n_sexes, data$n_srv_fleets, 3))
parameters$FishLen_corr_pars_agg <- array(0.01, dim = c(data$n_fish_fleets))
parameters$FishLen_corr_pars <- array(0.01, dim = c(data$n_regions, data$n_sexes, data$n_fish_fleets, 3))
parameters$FishAge_corr_pars_agg <- array(0.01, dim = c(data$n_fish_fleets))
parameters$FishAge_corr_pars <- array(0.01, dim = c(data$n_regions, data$n_sexes, data$n_fish_fleets, 3))
...
# Mapping ---------------------------------------------------------------
mapping <- list()
mapping$ln_sigmaR <- factor(c(NA, 1))
...
mapping$ln_F_devs <- factor(c(1:length(data$years), rep(NA, 3), 66:((length(data$years)*2)-3))) # fix all fishery mort parameters for now
...
# Fixing continuous time-varying selecitvity stuff
map_srvsel_pe_pars <- parameters$srvsel_pe_pars
map_srvsel_pe_pars[] = NA
mapping$srvsel_pe_pars = factor(map_srvsel_pe_pars)
# sel devs
map_ln_srvel_devs <- parameters$ln_srvsel_devs
map_ln_srvel_devs[map_ln_srvel_devs == 0] <- NA
mapping$ln_srvsel_devs <- factor(map_ln_srvel_devs)
data$map_ln_srvsel_devs <- array(as.numeric(mapping$ln_srvsel_devs), dim = dim(parameters$ln_srvsel_devs))
...
mapping$ln_FishAge_theta_agg <- factor(rep(NA, length(parameters$ln_FishAge_theta_agg)))
mapping$ln_FishLen_theta_agg <- factor(rep(NA, length(parameters$ln_FishLen_theta_agg)))
mapping$ln_SrvAge_theta_agg <- factor(rep(NA, length(parameters$ln_SrvAge_theta_agg)))
mapping$ln_SrvLen_theta_agg <- factor(rep(NA, length(parameters$ln_SrvLen_theta_agg)))

mapping$SrvLen_corr_pars_agg <- factor(rep(NA, length(parameters$SrvLen_corr_pars_agg)))
mapping$SrvLen_corr_pars <- factor(rep(NA, length(parameters$SrvLen_corr_pars)))
mapping$SrvAge_corr_pars_agg <- factor(rep(NA, length(parameters$SrvAge_corr_pars_agg)))
mapping$SrvAge_corr_pars <- factor(rep(NA, length(parameters$SrvAge_corr_pars)))
mapping$FishLen_corr_pars_agg <- factor(rep(NA, length(parameters$FishLen_corr_pars_agg)))
mapping$FishLen_corr_pars <- factor(rep(NA, length(parameters$FishLen_corr_pars)))
mapping$FishAge_corr_pars_agg <- factor(rep(NA, length(parameters$FishAge_corr_pars_agg)))
mapping$FishAge_corr_pars <- factor(rep(NA, length(parameters$FishAge_corr_pars)))
...
RTMB = c(
  ...
  sum(sabie_rtmb_model$rep$FishAgeComps_nLL),
  sum(sabie_rtmb_model$rep$FishLenComps_nLL[,,1,1]),
  sum(sabie_rtmb_model$rep$FishLenComps_nLL[,,2,1]),
  sum(sabie_rtmb_model$rep$FishLenComps_nLL[,,1,2]),
  sum(sabie_rtmb_model$rep$FishLenComps_nLL[,,2,2]),
  sum(sabie_rtmb_model$rep$SrvAgeComps_nLL[,,1,1]),
  sum(sabie_rtmb_model$rep$SrvLenComps_nLL[,,1,1]),
  sum(sabie_rtmb_model$rep$SrvLenComps_nLL[,,2,1]),
  sum(sabie_rtmb_model$rep$SrvLenComps_nLL[,,1,2]),
  sum(sabie_rtmb_model$rep$SrvLenComps_nLL[,,2,2]),
  sum(sabie_rtmb_model$rep$SrvLenComps_nLL[,,1,3]),
  sum(sabie_rtmb_model$rep$SrvLenComps_nLL[,,2,3]),
  sum(sabie_rtmb_model$rep$Catch_nLL) * data$Wt_Catch,
  sum(sabie_rtmb_model$rep$SrvIdx_nLL[1,,1]) * data$Wt_SrvIdx,
  sum(sabie_rtmb_model$rep$SrvIdx_nLL[1,,3]) * data$Wt_SrvIdx,
  sum(sabie_rtmb_model$rep$SrvIdx_nLL[1,,2]) * data$Wt_SrvIdx,
  ...
  sum(sabie_rtmb_model$rep$Fmort_nLL) * data$Wt_F,
  sabie_rtmb_model$rep$M_nLL,
...
# Read in data
tem_dat <- dget(here("dev", '2024 Base (23.5)_final model_v2', 'tem.rdat'))
ageing_dat <- dget(here("dev",'2023 Base (23.5)_final model', 'test.rdat')) # for getting ageing error
tem_admb_dat <- readLines(here("dev","2024 Base (23.5)_final model_v2", "tem_2024_na_wh.dat"))
tem_rep <- readLines(here("dev",'2024 Base (23.5)_final model_v2', 'sable.rep'))
tem_par <- read_pars(here("dev",'2024 Base (23.5)_final model_v2', 'tem'))
...
data$Tag_LikeType <- 0 # poisson likelihood
data$mixing_period <- 0 # when to start mixing period after first release year
data$t_tagging <-1 # discounting for tagging 
data$tag_natmort <- NA # how to apply natural mortality to tags
...
# Change initial conditions to be derived based on cumulative mortality
# Try to freely estimate all sex-specific selectivity parameters (with minimal sharing)
...
unopt_ts_df <- rbind(
  unopt_ts_df_v1 %>% mutate(Comparison = "ADMB (23.5)"),
  unopt_ts_df_v2 %>% mutate(Comparison = "ADMB (23.5a; no dev vector)"),
  unopt_ts_df_v3 %>% mutate(Comparison = "ADMB (23.5b; no dev vector and max)")
) %>% 
  mutate(Comparison = factor(Comparison,
                             levels = c('ADMB (23.5)', "ADMB (23.5a; no dev vector)",
                                        "ADMB (23.5b; no dev vector and max)")))
...
png(here("output", "Bridging", "unopt_re_ts.png"), width = 1300, height = 800)
unopt_re_ts
dev.off()
...
png(here("output", "Bridging", "unopt_ts.png"), width = 1300, height = 800)
unopt_ts
dev.off()


unopt_sel_df <- rbind(
  unopt_combined_sel_v1 %>% mutate(Comparison = "ADMB (23.5)"),
  unopt_combined_sel_v2 %>% mutate(Comparison = "ADMB (23.5a; no dev vector)"),
  unopt_combined_sel_v3 %>% mutate(Comparison = "ADMB (23.5b; no dev vector and max)")
) %>% 
  mutate(Comparison = factor(Comparison,
                             levels = c('ADMB (23.5)', "ADMB (23.5a; no dev vector)",
                                        "ADMB (23.5b; no dev vector and max)")))
...
# Time series estimates
opt_ts_df <- rbind(
  opt_ts_df_v1 %>% mutate(Comparison = "ADMB (23.5)"),
  opt_ts_df_v2 %>% mutate(Comparison = "ADMB (23.5a; no dev vector)"),
  opt_ts_df_v3 %>% mutate(Comparison = "ADMB (23.5b; no dev vector and max)")
) %>% 
  mutate(Comparison = factor(Comparison,
                             levels = c('ADMB (23.5)', "ADMB (23.5a; no dev vector)",
                                        "ADMB (23.5b; no dev vector and max)")))
...
png(here("output", "Bridging", "opt_re_ts.png"), width = 1300, height = 1100)
opt_re_ts
dev.off()

# ADMB models
admb_ts <- ggplot(opt_ts_df, aes(x = Year, y = ADMB, color = Comparison)) +
  geom_line(size = 1.4) +
  facet_wrap(~Par, scales = "free_y") +
  labs(y = "Value") +
  theme_sablefish()

png(here("output", "Bridging", "ADMB_ts.png"), width = 1300, height = 800)
admb_ts
dev.off()
...
png(here("output", "Bridging", "opt_ts.png"), width = 1300, height = 1000)
opt_ts
dev.off()

opt_sel_df <- rbind(
  opt_combined_sel_v1 %>% mutate(Comparison = "ADMB (23.5)"),
  opt_combined_sel_v2 %>% mutate(Comparison = "ADMB (23.5a; no dev vector)"),
  opt_combined_sel_v3 %>% mutate(Comparison = "ADMB (23.5b; no dev vector and max)")
) %>% 
  mutate(Comparison = factor(Comparison,
                             levels = c('ADMB (23.5)', "ADMB (23.5a; no dev vector)",
                                        "ADMB (23.5b; no dev vector and max)")))
...
par_df <- rbind(
  par_df_v1 %>% mutate(Comparison = "ADMB (23.5)"),
  par_df_v2 %>% mutate(Comparison = "ADMB (23.5a; no dev vector)"),
  par_df_v3 %>% mutate(Comparison = "ADMB (23.5b; no dev vector and max)")
) %>% 
  mutate(Comparison = factor(Comparison,
                             levels = c('ADMB (23.5)', "ADMB (23.5a; no dev vector)",
                                        "ADMB (23.5b; no dev vector and max)")))
