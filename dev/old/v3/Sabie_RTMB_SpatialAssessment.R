...
data$Wt_FishAgeComps <- array(NA, dim = c(data$n_regions, length(input_list$data$years), data$n_sexes, data$n_fish_fleets)) # weights for fishery age comps
...
data$Wt_FishLenComps <- array(NA, dim = c(data$n_regions, length(input_list$data$years), data$n_sexes, data$n_fish_fleets)) # weights for fish len comps
data$Wt_FishLenComps[,1,] <- 0 # Weight for fish len comps
...
data$Wt_SrvAgeComps <- array(NA, dim = c(data$n_regions, length(input_list$data$years), data$n_sexes, data$n_srv_fleets)) # weights for survey age comps
...
data$Wt_SrvLenComps <- array(0, dim = c(data$n_regions, length(input_list$data$years), data$n_sexes, data$n_srv_fleets)) # weights for survey age comps
...
data$tag_selex = 5 # use dominant fleet seletivity and sex-specific
data$mixing_period <- 2 # when to start mixing period after first release year
data$t_tagging <- 0.5 # discounting for tagging
data$tag_natmort <- 3 # region, age-specific natural mortlaity, but with sex-aggregated natural mortality
...
data$TagRep_sd = 5 # sd of tag reporting rate prio
data$Tag_Reporting_blocks = array(0, dim = c(data$n_regions, length(data$years)))
data$Tag_Reporting_blocks[,(1960:1994)-1959] <- 1 # pre-IFQ
data$Tag_Reporting_blocks[,(1995:2016)-1959] <- 2 # post-IFQ
data$Tag_Reporting_blocks[,(2017:2021)-1959] <- 2 # pot fleet transition (based on earlier analyses, probably no need to acocunt for that)

# movement rate priors
data$Use_Movement_Prior = 1 # use mvoement rate prior
data$Movement_prior = array(1.5, dim = c(data$n_regions, data$n_regions, length(data$years), length(data$ages), data$n_sexes)) # away form edges
...
parameters$ln_sigmaR <- c(log(0.4), log(1.2))

# Comp likelihood stuff (not used)
parameters$ln_FishAge_theta <- array(0, dim = c(data$n_regions, data$n_fish_fleets, data$n_sexes))
parameters$ln_FishLen_theta <- array(0, dim = c(data$n_regions, data$n_fish_fleets, data$n_sexes))
parameters$ln_SrvAge_theta <- array(0, dim = c(data$n_regions, data$n_srv_fleets, data$n_sexes))
parameters$ln_SrvLen_theta <- array(0, dim = c(data$n_regions, data$n_srv_fleets, data$n_sexes))
...
parameters$Tag_Reporting_Pars <- array(log(0.2 / (1 - 0.2)), dim = c(data$n_regions, max(apply(data$Tag_Reporting_blocks, 1, function(x) length(unique(x))))))
parameters$ln_tag_theta <- log(0.5)

# Mapping -----------------------------------------------------------------
mapping <- list()
mapping$dummy <- factor(NA)
mapping$ln_sigmaR <- factor(rep(NA, 2))
...
# Tag reporting rates (shared across regions)
map_Tag_Reporting_Pars <- parameters$Tag_Reporting_Pars
map_Tag_Reporting_Pars[,1] <- 1
map_Tag_Reporting_Pars[,2] <- 2
...
model_rep = sabie_rtmb_model$rep 
