...
input_list <- Setup_Mod_Movement(input_list = input_list,
                                 # Model options
                                 Movement_ageblk_spec = 'constant', # estimating movement in 3 age blocks
                                 ...
                                 cont_vary_movement = 'iid',
                                 Movement_cont_age_spec = 'none',
                                 Movement_cont_year_spec = 'est_all',
                                 Movement_cont_pe_pars_spec = 'est_shared_r_a'
)
...
data$cont_vary_movement <- 2
map_move_pe_pars <- parameters$move_pe_pars[]
map_move_pe_pars[] <- NA
map_move_pe_pars[1,c(2,4)] <- 1:2
parameters$move_pe_pars[1,c(2,4)] <- c(0.5, log(0.05))
mapping$move_pe_pars <- factor(map_move_pe_pars)

# make AD model function
obj <- RTMB::MakeADFun(SPoRC:::cmb(SPoRC:::SPoRC_rtmb, data), parameters = parameters,
                       map = mapping, silent = FALSE, random = 'logit_move_devs')
...
corr_mat <- reshape2::melt(cov2cor(obj$sd_rep$cov.fixed )) %>%
  filter(value != 1, Var1 == 'logit_move_devs')

par(mfrow = c(2,2))
plot(obj$rep$fish_sel[1,1,,1,2])
plot(obj$rep$fish_sel[1,1,,2,2])
plot(obj$rep$Rec[1,])
plot(obj$rep$Rec[2,])
plot(obj$rep$Rec[3,])

reshape2::melt(obj$rep$Movement[,,,,]) %>%
  rename(From = Var1, To = Var2, Year = Var3, Age = Var4, Sex = Var5) %>%
  filter(Age %in% c(2:8), From == 1) %>%
  ggplot(aes(x = Year + 1959, y = value, color = factor(Age))) +
  # geom_smooth() +
  geom_line() +
...
reshape2::melt(obj$rep$Movement[,,,,]) %>%
  rename(From = Var1, To = Var2, Year = Var3, Age = Var4, Sex = Var5) %>%
  # filter(From == 1) %>%
  ggplot(aes(x = Age, y = value, color = Year, group = Year)) +
  # geom_smooth() +
  geom_line() +
  # ylim(0,1)+
  facet_grid(From~To, scales = 'free')

reshape2::melt(obj$rep$Rec) %>%
  ggplot(aes(x = Var2 + 1959, y = value)) +
  # geom_smooth() +
  geom_line() +
  facet_wrap(~Var1)

reshape2::melt(obj$rep$SSB) %>%
...
get_idx_fits(data, obj$rep, year_labs = 1960:2021) %>%
  filter(obs != 0, Year >= 1995) %>%
  ggplot() +
